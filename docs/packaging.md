...
Default package console output stays compact. It shows `ComputerName`, `Status`, `ExitCode`, `Package`, `Cleanup`, `Logs`, and `ErrorSummary`; successful rows leave `ErrorSummary` blank. `Cleanup` summarizes staged-package cleanup as `Retained`, `Removed`, or `Failed`; `Logs` shows whether package log copy produced copied logs. Full package output, errors, cleanup metadata, copied-log paths, and artifact paths remain on the returned object and in root/per-target `run.log` files when captured.
