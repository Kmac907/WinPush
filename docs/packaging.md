...
- Direct `-ComputerName` arrays, pipeline strings, and pipeline objects with a `ComputerName` property are supported for package workflows; `-HostFile` package targets are rejected until the host-file package slice is implemented.
